...
PATH_TO_SLAVE_ZONEFILE = '/etc/bind/named.conf.local'
...
def __main__():
    ...
    masterzone = get_master_zones(MASTERIP, REMOTE_USERNAME, PATH_TO_MASTER_ZONEFILE)
    slavezone = get_slave_zones(PATH_TO_SLAVE_ZONEFILE)
    newslave = match_append(masterzone, slavezone)

    if newslave == 'same':
        print 'No new zonefiles'
    else:
        new_zonefile(newslave, PATH_TO_SLAVE_ZONEFILE, MASTERIP)
